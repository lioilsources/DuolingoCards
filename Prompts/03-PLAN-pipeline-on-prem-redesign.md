Uprav pipeline a mobilní appku podle plánu. 
...
1. **TTS fallback** — když chybí OS hlas pro jazyk: skrýt tlačítko, nebo vyzvat ke stažení hlasu?
1. **RevenueCat vs čistý StoreKit/Billing** — chceš jednotnou evidenci napříč platformami, nebo to držet úplně bez závislosti?
1. **Cenový model** — free trychtýř + bundly (doporučeno) vs à la carte vs subscription. (Beze změny z dodatku.)
1. **Multi-styl** — jeden vizuál teď, nebo rovnou víc stylů / stejný deck ve dvou vizuálech? Model i export to umožní kdykoli; teď stačí default styl.
1. **Pivot pro fakta** — en nebo cs jako jazyk, ve kterém autoruješ a kontroluješ info/summary před překladem?